  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] px-4 py-8 md:px-8">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          <label className="mb-2 block text-sm font-medium text-slate-700">Select Auditorium</label>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {auditoriums.map((a) => (
              <button
                ...
                className={`rounded-lg border p-3 text-left ${selectedId === a._id ? "border-slate-900" : "border-slate-200"}`}
              >
                {a.coverImage ? (
                  <img
                    src={a.coverImage}
        ...
        {selected && (
          <div className="rounded-2xl border bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
              <input className="rounded-md border px-3 py-2 text-sm" type="date" value={form.startDate} onChange={(e) => setForm((p) => ({ ...p, startDate: e.target.value }))} />
              <input className="rounded-md border px-3 py-2 text-sm" type="date" value={form.endDate} onChange={(e) => setForm((p) => ({ ...p, endDate: e.target.value }))} />
              <input className="rounded-md border px-3 py-2 text-sm" placeholder="Event name" value={form.eventName} onChange={(e) => setForm((p) => ({ ...p, eventName: e.target.value }))} />
              <input className="rounded-md border px-3 py-2 text-sm" type="number" min={1} placeholder="Expected attendees" value={form.attendeeCount} onChange={(e) => setForm((p) => ({ ...p, attendeeCount: e.target.value }))} />
              <input className="rounded-md border px-3 py-2 text-sm" placeholder="Your full name" value={form.bookedByName} onChange={(e) => setForm((p) => ({ ...p, bookedByName: e.target.value }))} />
              <input className="rounded-md border px-3 py-2 text-sm" placeholder="Phone number" value={form.bookedByPhone} onChange={(e) => setForm((p) => ({ ...p, bookedByPhone: e.target.value }))} />
              <input className="md:col-span-2 rounded-md border px-3 py-2 text-sm" type="email" placeholder="Email (optional)" value={form.bookedByEmail} onChange={(e) => setForm((p) => ({ ...p, bookedByEmail: e.target.value }))} />
              <textarea className="md:col-span-2 rounded-md border px-3 py-2 text-sm" rows={3} placeholder="Additional notes (optional)" value={form.notes} onChange={(e) => setForm((p) => ({ ...p, notes: e.target.value }))} />
            </div>
            <div className="mt-4 flex items-center gap-3">
              <button disabled={loading} onClick={() => void checkAndBook()} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-60">
